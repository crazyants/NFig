using System;
using System.Collections.Generic;
using System.Security.Cryptography;
using NFig.Encryption;
using NFig.InMemory;
using NFig.Tests.FactoryTests;
using NUnit.Framework;

namespace NFig.Tests
{
    public class EncryptedSettingsTests
    {
        const string USER_A = "Andrew";
        const string USER_B = "Bret";
        const string USER_C = "Charlie";

        class MultipleAttributesSettings : SettingsBase
        {
            [EncryptedSetting]
            [Setting(null)]
            public string BadSetting { get; private set; }
        }

        [Test]
        public void MultipleAttributes()
        {
            Assert.Throws<NFigException>(() =>
            {
                Utils.CreateStore<MultipleAttributesSettings>(encryptor: new PassThroughSettingEncryptor());
            });
        }

        class NoTierDefaultsSettings : SettingsBase
        {
            [EncryptedSetting]
            [DataCenter(DataCenter.Local, "value")]
            public string BadSetting { get; private set; }
        }

        [Test]
        public void NoTierDefaults()
        {
            Assert.Throws<NFigException>(() =>
            {
                Utils.CreateStore<NoTierDefaultsSettings>(encryptor: new PassThroughSettingEncryptor());
            });
        }

        [Test]
        public void InvalidEncryptor()
        {
            Assert.Throws<NFigException>(() =>
            {
                Utils.CreateStore<SettingsBase>(encryptor: new InvalidSettingEncryptor());
            });
        }

        [Test]
        public void PassThroughEncryption()
        {
            var store = Utils.CreateStore<Settings>(encryptor: new PassThroughSettingEncryptor());
            var settings = store.GetSettingsForGlobalApp();

            Assert.AreEqual("Plain text value", settings.EncryptedString);
            Assert.AreEqual("More plain text", settings.UnencryptedString);
            Assert.AreEqual(3, settings.EncryptedList.Length);
            Assert.AreEqual(1, settings.EncryptedList[0]);
            Assert.AreEqual(2, settings.EncryptedList[1]);
            Assert.AreEqual(3, settings.EncryptedList[2]);
            Assert.AreEqual(0, settings.EncryptedInt);
            Assert.AreEqual(17.3, settings.EncryptedDouble);
        }

#if NETFRAMEWORK
        [Test]
        public void RsaEncryption()
        {
            // if you're looking at this test as an example, remember, you SHOULD NOT store your real private key in source code,
            // since that defeats the purpose of encrypted settings in the first place.
            const string privateKey = "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent><P></P><Q></Q><DP></DP><DQ></DQ><InverseQ></InverseQ><D></D></RSAKeyValue>";
            var rcp = new RSACryptoServiceProvider();
            rcp.FromXmlString(privateKey);
            var rsa = new RsaSettingEncryptor(rcp, RSAEncryptionPadding.OaepSHA1);

            var store = Utils.CreateStore<Settings>(tier: Tier.Prod, dataCenter: DataCenter.East, encryptor: rsa);
            var settings = store.GetSettingsForGlobalApp();

            Assert.AreEqual("Encrypted text value", settings.EncryptedString);
            Assert.AreEqual("Also plain text", settings.UnencryptedString);
            Assert.AreEqual(3, settings.EncryptedList.Length);
            Assert.AreEqual(4, settings.EncryptedList[0]);
            Assert.AreEqual(5, settings.EncryptedList[1]);
            Assert.AreEqual(6, settings.EncryptedList[2]);
            Assert.AreEqual(0, settings.EncryptedInt);
            Assert.AreEqual(23.37, settings.EncryptedDouble);

            const string unencrypted = "Override text";
            const string encrypted = "";

            Assert.IsFalse(store.IsValidStringForSetting("EncryptedString", unencrypted));
            Assert.IsTrue(store.IsValidStringForSetting("EncryptedString", encrypted));
            Assert.IsTrue(store.IsValidStringForSetting("UnencryptedString", unencrypted));
            Assert.IsTrue(store.IsValidStringForSetting("UnencryptedString", encrypted));

            Assert.Throws<InvalidSettingValueException>(() =>
            {
                store.SetOverride("EncryptedString", unencrypted, DataCenter.Any, USER_A);
            });

            store.SetOverride("EncryptedString", encrypted, DataCenter.Any, USER_A);
            settings = store.GetSettingsForGlobalApp();
            Assert.AreEqual(unencrypted, settings.EncryptedString);
        }

        [Test]
        public void SymmetricEncryption()
        {
            // if you're looking at this test as an example, remember, you SHOULD NOT store your real encryption key in source code,
            // since that defeats the purpose of encrypted settings in the first place.
            var aes = new AesCryptoServiceProvider();
            aes.Key = new byte[] { 112, 202, 76, 140, 54, 135, 101, 171, 232, 82, 203, 123, 131, 161, 229, 37, 67, 79, 138, 88, 58, 121, 155, 133, 139, 97, 163, 169, 130, 247, 92, 242 };
            var encryptor = new SymmetricSettingEncryptor(aes);

            var store = Utils.CreateStore<Settings>(tier: Tier.Dev, dataCenter: DataCenter.East, encryptor: encryptor);
            var settings = store.GetSettingsForGlobalApp();

            Assert.AreEqual("AES encrypted value", settings.EncryptedString);
            Assert.AreEqual("So much plain text", settings.UnencryptedString);
            Assert.AreEqual(3, settings.EncryptedList.Length);
            Assert.AreEqual(7, settings.EncryptedList[0]);
            Assert.AreEqual(8, settings.EncryptedList[1]);
            Assert.AreEqual(9, settings.EncryptedList[2]);
            Assert.AreEqual(0, settings.EncryptedInt);
            Assert.AreEqual(89.95, settings.EncryptedDouble);

            const string unencrypted = "Override text";
            const string encrypted = "nrhmbztHkkgmHz96OM24vfwNgZMYM/It7jSdudRWQlc=";

            Assert.IsFalse(store.IsValidStringForSetting("EncryptedString", unencrypted));
            Assert.IsTrue(store.IsValidStringForSetting("EncryptedString", encrypted));
            Assert.IsTrue(store.IsValidStringForSetting("UnencryptedString", unencrypted));
            Assert.IsTrue(store.IsValidStringForSetting("UnencryptedString", encrypted));

            Assert.Throws<InvalidSettingValueException>(() =>
            {
                store.SetOverride("EncryptedString", unencrypted, DataCenter.Any, USER_A);
            });

            store.SetOverride("EncryptedString", encrypted, DataCenter.Any, USER_A);
            settings = store.GetSettingsForGlobalApp();
            Assert.AreEqual(unencrypted, settings.EncryptedString);
        }
#endif

        // Local for PassThrough testing
        // Dev for Symmetric (AES) testing
        // Prod for RSA testing
        class Settings : SettingsBase
        {
            [EncryptedSetting]
            [Tier(Tier.Local, "Plain text value")]
            // "AES encrypted value"
            [Tier(Tier.Dev, "VRY96vof1gkCVVIppgGiPC8KHRrDisr3b2M30NLDz4FtbA1EHt2RjkmSB/pSi8YL")]
            // "Encrypted text value"
            [Tier(Tier.Prod, "")]
            public string EncryptedString { get; private set; }

            [Setting(null)]
            [Tier(Tier.Local, "More plain text")]
            [Tier(Tier.Dev, "So much plain text")]
            [Tier(Tier.Prod, "Also plain text")]
            public string UnencryptedString { get; private set; }

            [EncryptedSetting]
            [Tier(Tier.Local, "1,2,3")]
            // "7,8,9"
            [Tier(Tier.Dev, "EEV8fzeEPfAl0epAUhpImaxCrS52p4yYOdSIxjPCvZU=")]
            // "4,5,6"
            [Tier(Tier.Prod, "")]
            [SettingConverter(typeof(CustomConverterTests.IntArrayConverter))]
            public int[] EncryptedList { get; private set; }

            [EncryptedSetting]
            public int EncryptedInt { get; private set; }

            [EncryptedSetting]
            [Tier(Tier.Local, "17.3")]
            // 89.95
            [Tier(Tier.Dev, "nn2SLeq2UVCUP4/Eba7/uzhNEPYwSRnUbSDo8sVql3c=")]
            // 23.37
            [Tier(Tier.Prod, "")]
            public double EncryptedDouble { get; private set; }
        }

        // an encryptor which doesn't round-trip properly
        class InvalidSettingEncryptor : ISettingEncryptor
        {
            public string Encrypt(string value)
            {
                return Guid.NewGuid().ToString();
            }

            public string Decrypt(string encryptedValue)
            {
                return Guid.NewGuid().ToString();
            }
        }
    }
}